...
export default function QuotesPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      // Only load quotes for current userId
      ...
      // We used sortBy + reverse because Dexie’s orderBy uses an index.
      // This keeps behaviour stable without requiring createdAt index changes.
    ...
    // Don’t load until we know which user we’re scoping to
    if (!userId) return;
  ...
  return (
    <div style={{ display: "grid", gap: 12 }}>
      ...
      <div className="top">
        ...
        <a href="/app/dashboard" className="btnSecondary">← Dashboard</a>
      ...
      <div className="panel">
        ...
        {filtered.length === 0 ? (
          <div style={{ padding: 14, opacity: 0.75 }}>
            No quotes yet for this account. Create your first one.
          </div>
        ) : (
          <>
            {filtered.map((qt) => (
